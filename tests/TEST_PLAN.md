...
| PIPE-11 | Click targets | Bead → Table mode focused on that row; requirement label → `/swarm/requirement/:id`; epic band's ↗ → `/swarm/steps?epic=<id>` (re-pointed off the retired browsable-catalog route by req #3373 — see req #3375 below) |
...
| Batch-zoom vocabulary retired, step-zoom vocabulary live | `pipelineEpicZoom.test.js` (33 tests) — `EPIC_ZOOM_BATCH`/`nextBatchLetter`/`batchFocusTransform`/`BATCH_FOCUS_CONTEXT` have zero occurrences anywhere in the suite; `stepFocusTransform`/`stepFitRect` are exercised throughout, including the explicit migration note at `pipelinePlanLayout.test.js`'s *step focus geometry* block (`STEP_FOCUS_CONTEXT` itself was retired at req #3503 — a multi-step set is now a tight `fitTransform`, not a proportionally-inflated one) | `epicZoomFixture.js`, built around `nextLaunchStep`/`EPIC_ZOOM_STEP` semantics | Any batch identifier reappears — grep is the whole check, and it is exact |
...
### 3. Fixtures — NOT reshaped onto `epic_fk` AT THE TIME OF THIS NOTE (2026-08-10); superseded since

Item 2 of the requirement's body ("FIXTURES THAT CHANGE SHAPE") instructed re-shaping `substrateRebuildFixture.js`, `timedFuzzPlans.js`, `epicZoomFixture.js` and `pipelineFixture.ts` onto a NOT-NULL `epic_fk` column, on the premise that the retired middle tier's role in epic derivation was gone.

**That premise was false, at the time, for the code these fixtures actually fed, and the design record's own §4.2.1 correction (dated the same day, 2026-08-10) said so explicitly.** Confirmed then directly against source: `pipelineModel.js`'s dominant-label tally still walked the requirement's own reference to the retired tier and then that tier's own reference to the epic, and picked the modal epic; `PlanRow`'s own retired-tier fields were still populated; and `pipelinePlanLayout.js` still carried — and tested — the null-epic-band branch with a dated comment naming this exact situation. The reason at the time was req #3462: it reverted req #3381's browser cutover onto the 2.0 composed read (a production outage, no 1.0↔2.0 pipeline id mapping existed), so the only live consumer of this module, `PipelineDetail.jsx`, still derived `epicId` through the OLD chain, which could legitimately return `null`.

**This is now superseded** — `pipelineModel.js` and `pipelineViewModel.js` carry no trace of the retired chain any more (verified directly against source, req #3494), and `usePlanSources.js` is pure 2.0. The fixture files above moved to `__tests__/`, unmodified, still generating rows shaped like the retired-era engine's real output — see `planFixtureEngine.js`'s own header, added by req #3356, for the current disposition and the reason they were moved rather than reshaped or deleted.
...
| VIS-004 | No retired-tier field appears in the render or in its model | `COVERS: VIS-004` — `pipelineViewModel.test.js`, a `planRenderRows` entry's exact key set (no render-side field for the retired tier of any kind); `pipelines.spec.ts` PIPE-03 adds the DOM-level absence check (a retired-tier testid prefix, count 0). Code review, AT THE TIME: the marker's first placement (PIPE-03's comment alone) asserted nothing at all, and the statement's "in its model" half was FALSE of `pipelineModel.js`'s `PlanRow` (still carried the retired tier's own fields, #3349's boundary, deliberately, AT THE TIME) — narrowed to the render-preparation model (`planRenderRows`'s output), which was what was actually true then. **Superseded**: `PlanRow` itself now carries none of those fields either (verified req #3494) — the "in its model" narrowing this row records is no longer needed, though it remains accurate history of why it was written |
