...
// pipeline2 sibling, not an import of it (see pipeline2ViewModel.js's header
 ...
 * `delete_pipeline2_epic` (darwin-mcp/services/pipelines2.py).
 *
 ...
 * every real epic: `pipeline2_step_deps.dep_step_fk` is ON DELETE RESTRICT,
...
export async function deleteEpic2(darwinUri, idToken, id) {
    const stepRows = await fetchEntity(
        `${darwinUri}/pipeline2_steps?epic_fk=${id}&fields=id`, idToken);
    ...
    if (stepIds.length) {
        ...
        const referencing = await fetchEntity(
            `${darwinUri}/pipeline2_step_deps?dep_step_fk=(${stepIds.join(',')})`
        ...
        if (internal.length) {
            ...
            // this session cannot read to verify (see steps2Api.js's header
            ...
            await Promise.all(internal.map((row) => call_rest_api(
                `${darwinUri}/pipeline2_step_deps`, 'DELETE', { id: row.id }, idToken)
                .then((r) => assertOk(r, 'deleteEpic2 (clearing an intra-epic dependency edge)'))));
        }
    }
