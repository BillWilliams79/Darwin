...
export function useRequirements(creatorFk, categoryId, { fields = 'id,title,requirement_status,scheduled,category_fk,sort_order,completed_at,deferred_at,started_at,coordination_type', enabled = true } = {}) {
    const { darwinUri } = useContext(AppContext);
    const { idToken } = useContext(AuthContext);
