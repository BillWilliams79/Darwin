// Mutation utilities for the pipeline2_steps data type (req #3393).
//
// Goes through the same generic gateway path darwin-mcp/services/pipelines2.py
// already POSTs/PUTs/DELETEs: `{darwinUri}/pipeline2_steps`. No new backend
// surface exists or is needed. Pattern mirrors Steps/stepsApi.js — this is its
// pipeline2 sibling, not an import of it.
//
...
// unless it is reproduced here. It is: `steps2Model.js` holds the decision as
// a pure tested function (`completionGuard`); `completeStep2` below is the
...
// — the two functions below (`fetchStep2RequirementIds`,
...
//   completed_at  nullable DATETIME — stamped by completeStep2, cleared by reopenStep2
 ...
 */
export async function createStep2(darwinUri, idToken,
    { epic_fk, title, run = 'auto', notes = null, not_before = null }) {
    const r = await call_rest_api(`${darwinUri}/pipeline2_steps`, 'POST',
        { epic_fk, title, run, notes, not_before, completed_at: null }, idToken);
    return assertOk(r, 'createStep2');
}
 ...
 * appends.
 */
export async function updateStep2(darwinUri, idToken, id, fields) {
    const r = await call_rest_api(`${darwinUri}/pipeline2_steps`, 'PUT',
        [{ id, ...fields }], idToken);
    return assertOk(r, 'updateStep2');
}
 ...
 */
export async function completeStep2(darwinUri, idToken, id) {
    return updateStep2(darwinUri, idToken, id, { completed_at: sqlNow() });
}

/** Clear `completed_at`. Unguarded — reopening only ever moves the stored
 * column towards what a derived step already reports. */
export async function reopenStep2(darwinUri, idToken, id) {
    return updateStep2(darwinUri, idToken, id, { completed_at: REST_NULL });
}

/**
 * HARD-delete a step, ONE statement — same single-statement rationale
 * `Steps/stepsApi.js`'s `deleteStep` states: `pipeline2_step_requirements` and
 * `pipeline2_step_deps` rows referencing this step as `step_fk` cascade with
 ...
 * partial write.
 */
export async function deleteStep2(darwinUri, idToken, id) {
    const r = await call_rest_api(`${darwinUri}/pipeline2_steps`, 'DELETE', { id }, idToken);
    return assertOk(r, 'deleteStep2');
}
 ...
 */
export async function fetchStep2RequirementIds(darwinUri, idToken, stepId) {
    const rows = await fetchEntity(
        `${darwinUri}/pipeline2_step_requirements?step_fk=${stepId}`, idToken);
 ...
 * gone — the caller must treat that as gating (see `steps2Model.js`).
 * `requirements` is era-agnostic (no `pipeline2_` prefix — it is shared core
